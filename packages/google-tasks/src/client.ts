...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['GOOGLE_TASKS_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['GOOGLE_TASKS_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Google Tasks API.
 */
export class GoogleTasks {
   ...
   * @param {string} [opts.baseURL=process.env['GOOGLE_TASKS_BASE_URL'] ?? https://tasks.googleapis.com/] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv("GOOGLE_TASKS_BASE_URL"),
    accessToken,
    ...opts
  }: ClientOptions) {
    if (accessToken === undefined) {
      throw new Errors.GoogleTasksError(
        "Missing required client option accessToken; you need to instantiate the GoogleTasks client with an accessToken option, like new GoogleTasks({ accessToken: 'My Access Token' }).",
      );
    }
    ...
    const options: ClientOptions = {
      accessToken,
      ...opts,
      baseURL: baseURL || `https://tasks.googleapis.com/`,
    };

    this.baseURL = options.baseURL!;
    this.timeout =
      options.timeout ?? GoogleTasks.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(
        readEnv("GOOGLE_TASKS_LOG"),
        "process.env['GOOGLE_TASKS_LOG']",
        this,
      ) ??
      defaultLogLevel;
  ...
  static GoogleTasks = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static GoogleTasksError = Errors.GoogleTasksError;
...
GoogleTasks.Tasks = Tasks;
export declare namespace GoogleTasks {
